            ...
            <div class="card-body">
                <table class="table table-responsive table-lg" class="display" id="tdroles">
                    ...
                    @foreach($roles as $role)
                        <tr class="">
                            <td>{{ $role->id ?? '' }}</td>
                            <td class="">{{ $role->label_role ?? '' }}</td>
                            <td class="">
                                @foreach($role->permissions as $key => $item)
                                    <span class="badge bg-light-primary my-1">{{ $item->label_permission }}</span>
                                @endforeach
                            </td>
                            ...
                            <td class="text-center">
                                ...
                                <div class="modal fade" id="modal{{ $role->id }}" tabindex="-1"
                                     aria-labelledby="deleterole" aria-hidden="true" style="display: none;" role="dialog">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ route('roles.destroy', $role->id_role) }}" method="POST">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    $role->label_role }}</span>. Cliquez sur "Confirmer" pour valider ou sur "Fermer" pour annuler... </p>
                                                    </p>
                                                </div>
